...
- **v0.17.0** — dependency bump: `go.viam.com/rdk v0.105.0 → v1.6.0`, `go.viam.com/api v0.1.496 → v0.1.577`, go directive `1.25.1 → 1.25.10`. No source change beyond the go.mod: rdk v1.6.0 moved `Status(ctx) (map[string]interface{}, error)` into `resource.Resource` and dropped `Reconfigure` from it (now `BuiltInResource.BuiltInReconfigure`, which viam-server calls only for its own internal resources), and `02eefbb` had already added `Status` to every fake ahead of the bump — so this is the go.mod half of a change started earlier. Exists because viamkit declared rdk v0.105.0 while consumers were resolving it to v1.x through MVS: the pairing built, but viamkit's own CI had never compiled or race-tested it, and `statemachine` / `watchdog` / `verify` / `worldstate` / `kinematics` / `cycle` are where most consumer behavior actually lives. `go test -race ./...` is green on all 14 packages at v1.6.0. Consumers inherit an rdk v1.6.0 floor and a Go 1.25.10 floor on their next `go mod tidy`.
